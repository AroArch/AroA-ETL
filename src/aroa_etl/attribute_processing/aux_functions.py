...
def keyword_removal(col, reg_expression):
    """This function removes specific keywords from a column"""

    return col.str.lower().apply(lambda x: re.sub(reg_expression, '', x))
...
def remove_name_spacing(string):


    pattern = re.compile(r'\b([A-Za-z])\s+(?=[A-Za-z]\b)')

    # remove spacing within words
    fixed_string = re.sub(pattern, r'\1', string)
...
def extend_female_last_name_endings(last_name_df):
    """
    This function autocompletes a female lastname ending if there is a value in the previous column.
    :param last_name_df: pd.DataFrame
    :return: pd.DataFrame
    """
    previous_column = last_name_df.columns[0]

    for column in last_name_df.iloc[:, 1:5]:

        for i, value in last_name_df[column].items():
            if value in ['Ova', 'Ene', 'Aite', 'Ite', 'Yte']:
                last_name_df.loc[i, column] = last_name_df.loc[
                                                  i,
                                                  previous_column] + value.lower()
...
def replace_raw_values(raw_value, matches):
    ...
    if len(matches) > 0:
        ...
        stand_value = raw_value
        for match in matches:
            stand_value = stand_value.replace(match[0].strip(), match[1])
        return stand_value

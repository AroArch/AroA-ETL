...
import numpy as np
import re
import aroa_etl.attribute_processing.regex_conditions as rc
import aroa_etl.attribute_processing.aux_functions as af
...
def default_processor(df,
                     column: str,
                     data_source: str = "crowd_sourcing"):
    """
        This function applies default processing to and qa flagging to a column. 
        Returns a DataFrame that contains the results.
    """
    col = df[column]

    # create qa column from string values 'Unklar and 'Unclear
    unclear_regex = r'[uU]nclear|[uU]nklar'
    ...
    clean_col = col.fillna('').astype(str)

    processed_df = pd.DataFrame({
            f"{column}_cleaned": clean_col,
            f"{column}_qa": qa_col,
            f"{column}_data_source": [data_source for i in range(len(qa_col))],
        }
    )
...
def normalize_last_name(df,
    ...
    name_col = name_col.str.lower().fillna('')

    # Remove blocked writing of names
    name_col = name_col.apply(lambda x: af.remove_name_spacing(x))
    ...
    name_col = af.keyword_removal(name_col, rc.key_words_regex)
    ...
    for element in name_df.columns:
        if alternative_spelling:
            ...
            rename_dict[element] = f'{col_name}_cleaned_{str(element)}'
    ...
    #create full name in column if only female last name ending is present.
    name_df = af.extend_female_last_name_endings(name_df)
...
def normalize_first_name(df,
    ...
    name_col = name_col.str.title().fillna('')
...
def normalise_date(df,
    ...
    # reformat dates in case of 6 or 8 digits present in case that separators were not given
    date_col = date_col.apply(
    ...
    is_null_df = date_df.isnull()[
        [f'{date_output_name}_day_cleaned',
         f'{date_output_name}_month_cleaned',
         f'{date_output_name}_year_cleaned']
    ]
    complete_date_qa = (
...
def date_correction(df,
    ...
    date_corrections = cp.normalise_date(
        df,
        date_col,
        output_name,
        data_source,
    ...
    comp_df.drop(
        comp_df[comp_df['comp'] == True].index,
        inplace=True
    )
...
def clean_wikidata_ids(df, wiki_col):
    """
    This function cleans the wikidata id columns. A QA column is created for uncertain entries.

    Parameters
    ----------
    df : pandas.DataFrame
        Dataframe containing columns with substring 'wikidata_id'.
    wiki_col : str
        Name of the 'wikidata_id' column.
    ...
    """
    col = df[wiki_col].fillna('').astype(str)

    # create qa column from string values 'Unklar and 'Unclear
    poi_regex = r'[uU][nN][cC][lL][eE][aA][rR]|[uU][nN][kK][lL][aA][rR]'
    ...
    def parse_wid(wid):
        wid = str(wid).strip()
        if re.match(r"^[qQ]\d+$",wid):
            return wid
        elif re.match(r"^\d+$",wid):
            return f"Q{wid}"
        return ''
    
    df[wiki_col] = col.apply(parse_wid)
    
    qa_col = qa_col | df[wiki_col] != col
    df[f"{wiki_col}_qa"] = qa_col

    return df
    
def clean_geoname_ids(df, geo_col):
    """
    This function cleans the geoname id columns. A QA column is created for uncertain entries.
    ...
    """
    col = df[geo_col].fillna('').astype(str)
    ...
    df[geo_col] = col.apply(parse_geoid)
    
    qa_col = qa_col | df[geo_col] != col
    df[f"{geo_col}_qa"] = qa_col
